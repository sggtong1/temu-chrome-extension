...
## 版本号规则(manifest.json `version`)

格式 `X.Y.Z`,改完代码先按这个 bump 再 commit:

| 位 | 触发条件 | 例子 |
|---|---|---|
| **Z**(第三位) | 小修改:文案、样式、一两行 bug fix | `1.1.0 → 1.1.1` |
| **Y**(第二位) | 大的修改:新功能、几个文件联动、UX 升级、可见行为变化 | `1.1.0 → 1.2.0` |
| **X**(第一位) | 非常重要:架构重写、协议改变、不向下兼容 | `1.x.x → 2.0.0` |

Bump Y 或 X 时,Z 归 0;Bump X 时 Y/Z 都归 0。

`background/agent.js` 内的 `AGENT_BUILD_ID` 是开发标记(每次改 agent.js 都要 bump,跟 manifest version 解耦),用于 SW 日志快速识别正在跑的代码版本。
