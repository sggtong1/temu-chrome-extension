...
数据存到本机的 `mini-postgres` 容器（database `ecommerce`，端口 5432）。
浏览器无法直连 PG wire protocol，需要前置一层 HTTP API —— 用 PostgREST
（与 Supabase 同源的 REST-over-PG 包装）。
...
脚本默认监听 `http://localhost:3002`，扩展通过这个 URL 调 `/dashboard_metrics`、
`/sku_daily_metrics`、`/ad_spend_daily`、`/sku_activity_history` 等表。
...
打开扩展选项页，填入：
- **API URL**：`http://localhost:3002`（默认值已预填）

本地开发不需要 anon key —— PostgREST 以 `admin` 作为 anon role，无 JWT 校验。
