...
```bash
bash scripts/start-api.sh         # 默认绑 0.0.0.0:3002, 局域网共享
BIND=127.0.0.1 bash scripts/start-api.sh   # 仅本机, 不让 LAN 其他人访问
```

脚本完成后输出可用 URL，例如：
- `http://yyjrs-Mac-mini.local:3002` （mDNS 主机名, Mac/iOS 自动解析）
- `http://192.168.1.6:3002`           （LAN IP, 全平台可用但 DHCP 续约可能变）
...
- macOS 防火墙允许端口 3002 入站（默认 OFF 通常不用动；如果开了 ALF 防火墙需要把
  Docker 加白名单）

### 安全注意

PostgREST 当前以 `admin` 作为 anon role，**任何能连到 3002 的人都能 INSERT/DELETE
